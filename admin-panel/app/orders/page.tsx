  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800">Orders & Fulfillment Management</h1>
          <p className="text-slate-500 text-xs mt-1">Track incoming customer orders and update dispatch & delivery statuses.</p>
        </div>
        <button
          onClick={fetchOrders}
      ...
      <div className="space-y-4">
        {orders.length === 0 ? (
          <div className="bg-white p-12 text-center rounded-2xl border border-slate-200/80">
            <p className="text-slate-500 font-bold text-xs">No orders recorded yet.</p>
          </div>
        ) : (
          orders.map(order => (
            <div key={order.id} className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b text-xs">
                <div>
                  ...
                  <p className="text-slate-400 text-[11px] mt-0.5">Placed on {new Date(order.createdAt).toLocaleString()}</p>
                </div>

                <div className="mt-2 sm:mt-0 flex items-center space-x-3">
                  <span className="font-bold text-slate-600 text-xs">Status:</span>
                  <select
                    value={order.status}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs bg-slate-50 p-3 rounded-xl border border-slate-100">
                <div>
                  ...
                  <p className="font-bold text-slate-800">{order.customerName} ({order.phone || 'N/A'})</p>
                </div>
                <div>
              ...
              <div className="space-y-2 text-xs">
                <p className="font-bold text-slate-700">Order Items ({order.items?.length || 0}):</p>
