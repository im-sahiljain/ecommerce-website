...
export default function ProductListingForm({
  ...
  useEffect(() => {
    ...
    async function load() {
      const [productRes, lineRes, categoryRes] = await Promise.all([
        adminFetch("/api/products").then((res) => res.json()).catch(() => []),
        adminFetch("/api/product-lines").then((res) => res.json()).catch(() => []),
        adminFetch("/api/categories").then((res) => res.json()).catch(() => []),
      ]);
  ...
  const fillFromProduct = (p: Product, asCopy: boolean) => {
    ...
    setSku(asCopy ? `SKU-${Date.now().toString().slice(-6)}` : p.sku || `SKU-${Date.now().toString().slice(-6)}`);
    ...
    setIsNewLaunch(
      p.isNewLaunch !== undefined ? Boolean(p.isNewLaunch) : Boolean(p.badge?.includes("New")),
    );
    setIsSellingFast(
  ...
  const handleProductLineChange = (newLineId: string) => {
    ...
    setTheme((current) => (current && newThemes.includes(current) ? current : ""));
  };
  ...
  if (missing) {
    return (
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-8 text-center">
        <p className="text-sm font-bold text-slate-700">This product was not found.</p>
        <Link href="/admin/products" className="text-xs font-bold text-pink-600">
          Back to products
        </Link>
      </div>
    );
    ...
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-4">
      ...
      <div className="space-y-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs font-bold text-slate-700">Product Name</label>
            <input
              type="text"
              value={name}
              ...
              required
              className="w-full rounded-xl border bg-slate-50 px-3 py-2 text-xs font-semibold"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-bold text-slate-700">SKU Code</label>
            <input
              type="text"
              value={sku}
              ...
              required
              className="w-full rounded-xl border bg-slate-50 px-3 py-2 font-mono text-xs"
            />
          </div>
        </div>

        <div className="space-y-3 rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
          <h2 className="border-b pb-1 text-xs font-extrabold text-slate-800">
            Catalog Classification
          </h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">1. Product Line</label>
              <select
                value={productLineId}
                onChange={(e) => handleProductLineChange(e.target.value)}
                className="w-full rounded-xl border bg-white px-3 py-2 text-xs font-semibold focus:border-pink-400 focus:outline-none"
              >
            ...
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">2. Category</label>
              <select
                value={category}
            ...
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">3. Theme</label>
              <select
                value={theme}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div>
            <label className="mb-1 block text-xs font-bold text-slate-700">Selling Price (₹)</label>
            <input
              type="number"
              step="0.01"
              value={price}
          ...
          <div>
            <label className="mb-1 block text-xs font-bold text-slate-700">Original Price (₹)</label>
            <input
              type="number"
              step="0.01"
              value={originalPrice}
          ...
          <div>
            <label className="mb-1 block text-xs font-bold text-slate-700">Initial Stock Quantity</label>
            <input
              type="number"
              value={stockQuantity}
        ...
        <div className="space-y-3 rounded-2xl border border-slate-200/80 bg-slate-50/90 p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="flex items-center gap-1.5 text-xs font-extrabold text-slate-800">
                <ImageIcon className="h-4 w-4 text-pink-500" />
                Product Images Upload (Cloudinary)
              </h2>
              <p className="mt-0.5 text-[11px] text-slate-500">
                Cloudinary Folder:{" "}
                <code className="rounded bg-slate-200/80 px-1.5 py-0.5 font-mono text-[10px] font-bold text-slate-800">
                  Ecommerce / Products / {name.trim() || "(Product Name)"}
                </code>
              </p>
            </div>
            <span className="shrink-0 rounded-full border border-pink-200 bg-pink-100 px-2 py-0.5 text-[10px] font-bold text-pink-700">
              Max 2 MB / file
            </span>
          </div>

          {uploadError && (
            <div className="flex items-center justify-between rounded-xl border border-rose-200 bg-rose-50 p-2.5 text-xs font-semibold text-rose-700">
              <span>{uploadError}</span>
              <button type="button" onClick={() => setUploadError(null)} className="text-rose-500">
                <X className="h-3.5 w-3.5" />
              </button>
            </div>
          )}

          <div className="group relative cursor-pointer rounded-xl border-2 border-dashed border-slate-300 bg-white p-4 text-center transition hover:border-pink-400">
            <input
              type="file"
              multiple
              accept="image/*"
            ...
            <div className="flex flex-col items-center space-y-1">
              <Plus className="h-6 w-6 text-pink-500 transition group-hover:scale-110" />
            ...
            <div className="space-y-2 border-t border-slate-200 pt-2">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                {imagesList.map((imgUrl, idx) => (
                  <div
                    key={`${imgUrl}-${idx}`}
                    className={`flex flex-col justify-between overflow-hidden rounded-2xl border-2 bg-white shadow-xs ${
                  ...
                  >
                    <div className="relative aspect-square w-full overflow-hidden bg-slate-100">
                      ...
                      <span
                        className={`absolute left-1.5 top-1.5 rounded-full px-2 py-0.5 text-[10px] font-extrabold shadow-sm ${
                          idx === 0 ? "bg-pink-500 text-white" : "bg-slate-900/80 text-white"
                        }`}
                      >
                        {idx === 0 ? "Main (#1)" : `#${idx + 1}`}
                      </span>
                      <button
                        ...
                        className="absolute right-1.5 top-1.5 rounded-full bg-rose-600/90 p-1.5 text-white hover:bg-rose-700"
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </div>
                    <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50 p-1.5 text-xs">
                      <button
                        type="button"
                        onClick={() => handleMoveImage(idx, idx - 1)}
                        disabled={idx === 0}
                        className="rounded-lg border border-slate-200 bg-white p-1 text-slate-700 disabled:opacity-30"
                      >
                        <ArrowLeft className="h-3.5 w-3.5" />
                      </button>
                      {idx !== 0 ? (
                        <button
                          ...
                          className="flex items-center gap-1 rounded-lg bg-pink-100 px-2 py-1 text-[10px] font-extrabold text-pink-700 hover:bg-pink-200"
                        >
                          <Star className="h-3 w-3 fill-current" />
                          Set Main
                        </button>
                      ) : (
                        <span className="px-1 text-[10px] font-extrabold text-pink-600">Primary</span>
                      )}
                      <button
                        type="button"
                        onClick={() => handleMoveImage(idx, idx + 1)}
        ...
        <div className="space-y-3 rounded-2xl border border-slate-200/80 bg-slate-50/90 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">Card Corner Badges</label>
            ...
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">Age Group / Tag (Optional)</label>
              <input
                type="text"
                value={ageGroup}
            ...
            <div>
              <label className="mb-1 block text-[11px] font-bold text-slate-700">Material (Optional)</label>
              <input
                type="text"
                value={material}
        ...
        <div className="flex flex-col gap-4 pt-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-xl bg-pink-500 px-5 py-2 text-xs font-bold text-white shadow-xs hover:bg-pink-600 disabled:opacity-50"
            >
