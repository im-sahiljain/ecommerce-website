...
function ShopPageContent() {
  ...
  // Filters State
  const [selectedProductLineId, setSelectedProductLineId] = useState(initialProductLineId);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedTheme) {
      list = list.filter(p => p.theme.toLowerCase() === selectedTheme.toLowerCase());
    }
    if (selectedCategory) {
      list = list.filter(p => p.category.toLowerCase() === selectedCategory.toLowerCase());
    }
    if (selectedAge) {
      list = list.filter(p => p.ageGroup.toLowerCase() === selectedAge.toLowerCase());
    }
    if (selectedScent) {
      list = list.filter(p => p.attributes?.Scent?.toLowerCase().includes(selectedScent.toLowerCase()));
    }
    if (inStockOnly) {
    ...
    return list;
  }, [products, selectedProductLineId, selectedTheme, selectedCategory, selectedAge, selectedScent, inStockOnly, maxPrice, sortOrder]);

  const activeProductLine = productLines.find(l => l.id === selectedProductLineId);
      ...
      <div className="bg-gradient-to-r from-pink-50 via-yellow-50 to-sky-50 p-8 rounded-3xl border border-slate-100 soft-shadow mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-800 mt-1">
            {activeProductLine ? activeProductLine.name : 'Explore All Craft Kits & Candles'}
          </h1>
          <p className="text-xs text-slate-500 mt-1 max-w-xl">
            Browse non-toxic ready-to-paint plaster figurines, scented soy wax candles, and creative art sets.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 bg-white/80 backdrop-blur-xs p-1.5 rounded-full border border-slate-200 text-xs font-bold">
          <button
            ...
            className={`px-4 py-2 rounded-full transition ${
              selectedProductLineId === '' ? 'bg-pink-300 text-slate-800' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            All Lines
          </button>
          {productLines.map(line => (
            <button
              ...
              className={`px-4 py-2 rounded-full transition ${
                selectedProductLineId === line.id ? 'bg-pink-300 text-slate-800' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              {line.name}
            </button>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <aside className="w-full md:w-64 shrink-0 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 soft-shadow space-y-6">
            ...
            {/* Themes Filter */}
            <div>
              <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider mb-2">Themes</h4>
              <div className="space-y-1">
                {['', 'Space Adventures', 'Secret Garden (Floral)', 'Fairytale Magic', 'Wild Kingdom', 'Aesthetic', 'Classic'].map(t => (
                  <button
                    key={t || 'all-themes'}
            ...
            {selectedProductLineId === 'line-2' && (
              <div>
                <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider mb-2">Scent Type</h4>
                <div className="space-y-1">
                  {['', 'Scented', 'Unscented'].map(s => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-1.5 rounded-xl text-xs font-semibold transition ${
                        selectedScent === s ? 'bg-yellow-100 text-slate-800 font-bold' : 'text-slate-600 hover:bg-slate-50'
                      }`}
                    >
                      {s || 'All Scents'}
                    </button>
        ...
        <main className="flex-1 space-y-6">
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 text-xs">
            <span className="text-slate-500 font-medium">Showing <strong className="text-slate-800">{filteredProducts.length}</strong> items</span>
          ...
          {filteredProducts.length === 0 ? (
            <div className="bg-white p-12 text-center rounded-3xl border border-slate-100 space-y-3">
              <p className="text-slate-600 font-bold">No items found matching your active filters.</p>
              <button
                onClick={() => {
                  setSelectedProductLineId('');
                  setSelectedTheme('');
                ...
                <div key={product.id} className="bg-white rounded-3xl border border-slate-100 p-4 soft-shadow soft-shadow-hover transition flex flex-col justify-between">
                  <div>
                    <Link href={`/product/${product.id}`} className="block group">
                      ...
                      <h4 className="font-bold text-sm text-slate-800 line-clamp-1 mt-0.5 group-hover:text-pink-500 transition">{product.name}</h4>
                    </Link>
