...
import { ShieldCheck, Truck, CreditCard, Lock, CheckCircle2, MapPin, Plus, Check } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    if (!customerName || !addressLine || !city || !state || !zipCode) {
      setError('Please fill in your name and complete delivery address details.');
      return;
    }
    ...
    const fullShippingAddress = `${addressLine}, ${city}, ${state} ${zipCode}`.trim();
    ...
    return (
      <div className="max-w-2xl mx-auto my-16 p-8 bg-white rounded-3xl border border-slate-100 soft-shadow text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Thank you for ordering with Little Creators Craft Hub!</p>
        </div>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl text-left border border-slate-100 space-y-3 text-xs">
          ...
          <p><span className="font-bold text-slate-600">Customer:</span> {orderSuccess.customerName}</p>
          <p><span className="font-bold text-slate-600">Shipping Address:</span> {orderSuccess.shippingAddress}</p>
          <p><span className="font-bold text-slate-600">Tracking Code:</span> {orderSuccess.trackingNumber || 'TRK-88492019'}</p>
        </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-800">Checkout & Order Placement</h1>
        <p className="text-xs text-slate-500 mt-1">Select a saved shipping address or enter delivery details to place your craft order.</p>
      </div>
      ...
      {!user ? (
        <div className="bg-white p-8 rounded-3xl border border-slate-100 soft-shadow text-center space-y-4 max-w-lg mx-auto">
          ...
          <h3 className="text-lg font-bold text-slate-800">Account Authentication Required</h3>
          <p className="text-xs text-slate-500">Please log in to use your saved addresses and secure 1-click checkout.</p>
          <button
            onClick={openAuth}
        ...
        <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 soft-shadow space-y-4">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-extrabold text-xs text-slate-800">{addr.label || 'Home'}</span>
                          {addr.isDefault && (
                            <span className="px-2 py-0.5 bg-sky-100 text-sky-700 text-[10px] font-extrabold rounded-md">Default</span>
                          )}
                        </div>
                        <p className="text-xs font-bold text-slate-700 truncate">{addr.fullName}</p>
                        <p className="text-[11px] text-slate-500 line-clamp-2 mt-0.5">{addr.addressLine}, {addr.city}, {addr.state} {addr.zipCode}</p>
              ...
              <div className="pt-4 border-t border-slate-100 space-y-4">
                <h3 className="text-xs font-extrabold text-slate-400 uppercase tracking-wider">
                  {selectedAddressId === 'custom' ? 'Enter Delivery Details' : 'Selected Address Info'}
                </h3>
            ...
            >
              ...
              <span>{isSubmitting ? 'Processing Order...' : `Confirm & Place Order (₹${totalPrice.toFixed(2)})`}</span>
            </button>
          </div>

          {/* ORDER SUMMARY SIDEBAR */}
          <div className="bg-white p-6 rounded-3xl border border-slate-100 soft-shadow h-fit space-y-4">
            <h2 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3">Order Summary ({cart.length} Kits)</h2>
            <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
              {cart.map(item => (
                <div key={item.id} className="flex items-center justify-between text-xs pb-2 border-b border-slate-50">
